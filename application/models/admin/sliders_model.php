...
/*	Show all  */
	private function _get_query($param1 = NULL)
    {
      ...
      $column_search = array('slider_title');
		...
		if($param1 == 'show_list' && isset($_POST["length"]) && $_POST["length"] != -1)  
       	{  
            ...
            if($f_sql)
            { 
				return "SELECT * FROM `tbl_slider` WHERE $f_sql ORDER BY $order_by LIMIT $limit OFFSET $offset";
            }
            else
            {
				return "SELECT * FROM `tbl_slider` ORDER BY $order_by LIMIT $limit OFFSET $offset";	
            }
       	}  
       	else
       	{
       		if($f_sql)
            {
				return "SELECT * FROM `tbl_slider` WHERE $f_sql ORDER BY $order_by";
            }
            else
            {	
				return "SELECT * FROM `tbl_slider` ORDER BY $order_by";	
            }
       	}
    }
