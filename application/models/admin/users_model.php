...
/*	Show all  */
    ...
    {   
      ...
      $column_search = array('fuser_fname','fuser_email','fuser_phone'); 

    	$sql = array();
    	$f_sql = '';
      $order_by = 'fuser_id DESC';   
    	$sql[] = "fuser_status != '2'";
		...
		if($param1 == 'show_list' && isset($_POST["length"]) && $_POST["length"] != -1)  
       	{  
            ...
            if($f_sql)
            { 
				return "SELECT * FROM `tbl_fuser` WHERE $f_sql ORDER BY $order_by LIMIT $limit OFFSET $offset";
            }
            else
            {
				return "SELECT * FROM `tbl_fuser` ORDER BY $order_by LIMIT $limit OFFSET $offset";	
            }
       	}  
       	else
       	{
       		if($f_sql)
            {
				return "SELECT * FROM `tbl_fuser` WHERE $f_sql ORDER BY $order_by";
            }
            else
            {	
				return "SELECT * FROM `tbl_fuser` ORDER BY $order_by";	
            }
       	}
    }
