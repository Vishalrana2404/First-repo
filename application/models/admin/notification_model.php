...
/*	Show all  */
	private function _get_query($param1 = NULL)
    {
      ...
      $column_search = array('notification_title');

    	$sql = array();
    	$f_sql = '';
    	$order_by = 'notification_id DESC';   
    	$sql[] = "notification_status != '2'";
		...
		if($param1 == 'show_list' && isset($_POST["length"]) && $_POST["length"] != -1)  
       	{  
            ...
            if($f_sql)
            { 
				      return "SELECT * FROM `tbl_notification` WHERE $f_sql ORDER BY $order_by LIMIT $limit OFFSET $offset";
            }
            else
            {
				      return "SELECT * FROM `tbl_notification` ORDER BY $order_by LIMIT $limit OFFSET $offset";	
            }
       	}  
       	else
       	{
       		if($f_sql)
            {
				      return "SELECT * FROM `tbl_notification` WHERE $f_sql ORDER BY $order_by";
            }
            else
            {	
				      return "SELECT * FROM `tbl_notification` ORDER BY $order_by";	
            }
       	}
    }
