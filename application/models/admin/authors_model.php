...
/*	Show all  */
	private function _get_query($param1 = NULL)
    {
      ...
      $column_search = array('authors_title');

    	$sql = array();
    	$f_sql = '';
    	$order_by = 'authors_id DESC';   
    	$sql[] = "authors_status != '2'";
		...
		if($param1 == 'show_list' && isset($_POST["length"]) && $_POST["length"] != -1)  
       	{  
            ...
            if($f_sql)
            { 
				return "SELECT * FROM `tbl_authors` WHERE $f_sql ORDER BY $order_by LIMIT $limit OFFSET $offset";
            }
            else
            {
				return "SELECT * FROM `tbl_authors` ORDER BY $order_by LIMIT $limit OFFSET $offset";	
            }
       	}  
       	else
       	{
       		if($f_sql)
            {
				return "SELECT * FROM `tbl_authors` WHERE $f_sql ORDER BY $order_by";
            }
            else
            {	
				return "SELECT * FROM `tbl_authors` ORDER BY $order_by";	
            }
       	}
    }
